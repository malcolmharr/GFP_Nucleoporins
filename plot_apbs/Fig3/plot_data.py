...
clist_116 = plt.cm.jet(cmap_idx_116)
clist_98A = plt.cm.jet(cmap_idx_98A)

## Set up the plot
fig, ax = plt.subplots(nrows=2, dpi=300, figsize=[3,4])
ax[0].scatter(DelGhyd, logP116, s=10, c='g', lw=0.5, edgecolors='k')
ax[0].plot([-14000,-9000], LinFit(np.array([-14000,-9000]), *popt116), c='g', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt116[0], popt116[1]))
ax[0].plot([-14000,-9000], np.exp((np.array([-14000,-9000])/2.5)), c='k', lw=0.75, ls='--')
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[0].plot([DelGhyd[i]]*2, [pred116[i],logP116[i]], c=clist_116[i], lw=0.5)
ax[0].set_xlabel('')
ax[0].set_xlim(-14000,-9000)
ax[0].set_xticks([-14000,-12500,-10500,-9000])
...
ax[1].plot([-14000,-9000], LinFit(np.array([-14000,-9000]), *popt98A), c='darkorchid', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt98A[0], popt98A[1]))
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[1].plot([DelGhyd[i]]*2, [pred98A[i],logP98A[i]], c=clist_98A[i], lw=0.5)
ax[1].set_xlabel(r'$\Delta G_{hyd}$ (kJ/mol)', fontsize=10)
ax[1].set_xlim(-14000,-9000)
...
## Save the figure
plt.savefig('P_Ghyd_plotsv2.pdf')
plt.savefig('P_Ghyd_plotsv2.png')
plt.show()
