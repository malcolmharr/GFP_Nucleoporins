...
ax[0].plot([-4350,-3450], LinFit(np.array([-4350,-3450]), *popt116), c='g', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt116[0], popt116[1]))
ax[0].plot([-4350,-3450], np.exp((np.array([-4350,-3450])/2.5)), c='k', lw=0.75, ls='--')
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[0].plot([mean_dg[i]]*2, [pred116[i],logP116[i]], c=clist_116[i], lw=0.5)
ax[0].set_xlabel('')
ax[0].set_xlim(-4350,-3450)
ax[0].set_xticks([-4350,-4050,-3750,-3450])
...
ax[1].plot([-4350,-3450], LinFit(np.array([-4350,-3450]), *popt98A), c='darkorchid', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt98A[0], popt98A[1]))
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[1].plot([mean_dg[i]]*2, [pred98A[i],logP98A[i]], c=clist_98A[i], lw=0.5)
ax[1].set_xlabel(r'$\Delta G_{hyd}$ (kJ/mol)', fontsize=10)
ax[1].set_xlim(-4350,-3450)
