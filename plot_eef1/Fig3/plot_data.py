...
ax[0].plot([-10050,-9050], LinFit(np.array([-10050,-9050]), *popt116), c='g', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt116[0], popt116[1]))
ax[0].plot([-10050,-9050], np.exp((np.array([-10050,-9050])/2.5)), c='k', lw=0.75, ls='--')
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[0].plot([DelGhyd[i]]*2, [pred116[i],logP116[i]], c=clist_116[i], lw=0.5)
ax[0].set_xlabel('')
ax[0].set_xlim(-10050,-9050)
ax[0].set_xticks([-10000,-9750,-9500,-9250])
...
ax[1].plot([-10050,-9050], LinFit(np.array([-10050,-9050]), *popt98A), c='darkorchid', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt98A[0], popt98A[1]))
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[1].plot([DelGhyd[i]]*2, [pred98A[i],logP98A[i]], c=clist_98A[i], lw=0.5)
ax[1].set_xlabel(r'$\Delta G_{hyd}$ (kJ/mol)', fontsize=10)
ax[1].set_xlim(-10050,-9050)
