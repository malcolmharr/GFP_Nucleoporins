...
pred116 = LinFit(mean_dg, *popt116)
pred98A = LinFit(mean_dg, *popt98A)
...
ax[0].scatter(mean_dg, logP116, s=10, c='g', lw=0.5, edgecolors='k')
ax[0].plot([-6550,-4000], LinFit(np.array([-6550,-4000]), *popt116), c='g', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt116[0], popt116[1]))
ax[0].plot([-6550,-4000], np.exp((np.array([-6550,-4000])/2.5)), c='k', lw=0.75, ls='--')
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[0].plot([mean_dg[i]]*2, [pred116[i],logP116[i]], c=clist_116[i], lw=0.5)
ax[0].set_xlabel('')
ax[0].set_xlim(-6550,-4000)
ax[0].set_xticks([-6550,-5925,-5300,-4675,-4000])
...
ax[1].scatter(mean_dg, logP98A, s=10, c='darkorchid', lw=0.5, edgecolor='k')
ax[1].plot([-6550,-4000], LinFit(np.array([-6550,-4000]), *popt98A), c='darkorchid', lw=1, label=r'ln(P)=%.4f$\Delta G_{hyd}$+%.1f'%(popt98A[0], popt98A[1]))
## Plot residuals
for i in range(len(residual_116)):
    print(i)
    ax[1].plot([mean_dg[i]]*2, [pred98A[i],logP98A[i]], c=clist_98A[i], lw=0.5)
ax[1].set_xlabel(r'$\Delta G_{hyd}$ (kJ/mol)', fontsize=10)
ax[1].set_xlim(-4650,-4000)
...
## Save the figure
plt.savefig('P_Ghyd_plots_v3.pdf')
plt.savefig('P_Ghyd_plots_v3.png')
plt.show()
